...
**Goal:** [doxygen/direction-documentation-plan.md](doxygen-documentation-plan.md) 의 6개 Phase 를 실행하여 `cmake --build build_Darwin --target doxygen` 으로 모듈 API + 다이어그램 + 빌드 가이드를 포함한 HTML 문서를 생성한다. Phase 7 에서 신규 모듈/메서드의 Doxygen 보강.

**Architecture:** Phase 1~5 는 `src/<module>/<file>.h` 에 표준 Doxygen 주석을 추가하고 `.cpp` 의 비-자명한 인라인 주석만 정리. Phase 6 은 `doc/Doxyfile.in` 에 Graphviz/페이지 옵션 추가 + `doxygen/pages/` 마크다운으로 메인페이지·빌드가이드·의존성 문서를 작성하고 `\dot ... \enddot` 디렉티브로 레이어/시퀀스 다이어그램을 임베드.
 ...
 *          마이그레이션 계획은 [migration-plan.md](../../doxygen/migration-plan.md) 참조.
 */
namespace SJH {}
```
...
#  현재는 doxygen/pages/00-mainpage.md 가 \mainpage 디렉티브 보유
...
### Task 6.3: `doxygen/pages/` 디렉토리 + 메인 페이지 작성

**Files:**
- Create: `doxygen/pages/00-mainpage.md`

- [ ] **Step 6.3.1: 디렉토리 생성 확인**

Run: `mkdir -p doxygen/pages`
Expected: 에러 없음 (idempotent).

- [ ] **Step 6.3.2: 메인 페이지 작성**

Create: `doxygen/pages/00-mainpage.md` with content:
...
**Files:**
- Create: `doxygen/pages/10-build-system.md`

- [ ] **Step 6.4.1: 페이지 작성**

Create: `doxygen/pages/10-build-system.md` with content:
...
**Files:**
- Create: `doxygen/pages/20-dependencies.md`

- [ ] **Step 6.5.1: 페이지 작성**

Create: `doxygen/pages/20-dependencies.md` with content:
...
```bash
git add cmake/Doxygen.cmake doc/Doxyfile.in doxygen/pages/ .gitignore
git commit -m "$(cat <<'EOF'
[doc] : Phase 6 — diagrams + integrated pages

- Doxyfile: HAVE_DOT/SVG/collaboration graph 활성화
- doxygen/pages/ 신설: mainpage(레이어/시퀀스 \dot) + build-system + dependencies
...
```bash
git add src/object/camera.h src/context/context.h doxygen/direction-documentation-plan.md doxygen/direction-documentation-implementation-plan.md
git commit -m "$(cat <<'EOF'
...
- doxygen/direction-documentation-{plan,implementation-plan}.md: Phase 7 추가, Phase 1~6 완료 표시
EOF
)"
```
