#ifndef __SJH_FSM_OBJECT_STATE_MACHINE_H__
#define __SJH_FSM_OBJECT_STATE_MACHINE_H__

#include "fsm/i_fsm_state.h"
...
namespace SJH::FSM
{
    ...
    class ObjectStateMachine : public SJH::Scene::Component
    {
    public:
        ...
        ///                반드시 NONE 외의 값을 전달.
        ObjectStateMachine(TOwner& owner, TState startup = TState::NONE)
    ...
    private:
        bool TryTransitImpl(TTransit t)
        {
            ...
            if (curr == 0) return false;                          // I1: NONE 에서는 전이 불가
            if ((allowedFrom & curr) != curr) return false;       // I2: from-bit 검증
            auto targetIt = targetOf_.find((StateU)t);
            if (targetIt == targetOf_.end()) return false;        // target 미등록
            const TState target = targetIt->second;
            auto curStateIt = states_.find(curr);                 // I3: OnExit → swap → OnEnter
...
#endif // __SJH_FSM_OBJECT_STATE_MACHINE_H__
