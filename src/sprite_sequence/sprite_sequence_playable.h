...
#include "sprite/sprite_component.h"          // SJH::Sprite::SpriteComponent (frameIdx 갱신 대상)

namespace SJH::SpriteSequence
{
    /// @brief Sprite atlas frame 시퀀스 — PlayableBase.elapsed_ 흡수, OnUpdate 가 frame index 계산.
    /// @note  binding 주입: SpriteComponent* + SpriteFrameClip*. 둘 다 외부 owner (Playable 은 raw ptr 만).
