...
namespace SJH
{
	namespace
	{
		...
		void SetStencilToggle(bool want, Scene::StencilState &lastStencil, bool initialized)
		{
			if (initialized && lastStencil.Enabled == want)
				return;
			want ? glEnable(GL_STENCIL_TEST) : glDisable(GL_STENCIL_TEST);
			lastStencil.Enabled = want;
		}

		/// @brief 결정 2 — `glStencilFunc(Func, Ref, TestMask)` 한 호출의 3 인자 묶음.
		void SetStencilFunc(const Scene::StencilState &s, Scene::StencilState &last, bool initialized)
		{
			if (initialized && last.Func == s.Func && last.Ref == s.Ref && last.TestMask == s.TestMask)
				return;
			glStencilFunc(s.Func, s.Ref, s.TestMask);
			last.Func = s.Func;
			last.Ref = s.Ref;
			last.TestMask = s.TestMask;
		}

		/// @brief 결정 3 — `glStencilOp(SFail, DpFail, DpPass)` 한 호출의 3 인자 묶음.
		void SetStencilOp(const Scene::StencilState &s, Scene::StencilState &last, bool initialized)
		{
			if (initialized && last.SFail == s.SFail && last.DpFail == s.DpFail && last.DpPass == s.DpPass)
				return;
			glStencilOp(s.SFail, s.DpFail, s.DpPass);
			last.SFail = s.SFail;
			last.DpFail = s.DpFail;
			last.DpPass = s.DpPass;
		}

		/// @brief 결정 4 — `glStencilMask(WriteMask)` (0x00 = 읽기 전용).
		void SetStencilMask(const Scene::StencilState &s, Scene::StencilState &last, bool initialized)
		{
			if (initialized && last.WriteMask == s.WriteMask)
				return;
			glStencilMask(s.WriteMask);
			last.WriteMask = s.WriteMask;
		}

		/// @brief Stencil state 일괄 적용 — 결정 순서만 표현.
		void ApplyStencil(const Scene::StencilState &s, Scene::StencilState &last, bool initialized)
		{
			SetStencilToggle(s.Enabled, last, initialized);
		...
		/// @brief Depth state 일괄 적용 — 결정 순서만 표현.
	...
	void PipelineStateSetter::Set(const Pass::PipelineState &want,
	                              const Scene::StencilState &stencilOverride)
	{
		ApplyStencil(stencilOverride, mLastStencil, mInitialized);
