 ...
 * @brief Pass::PipelineState 를 GL state 로 적용 — last-applied dirty 캐싱.
 *
 * @details
 *  ### 책임 (SP-PipelineSetter, SP-MaterialSSoT)
 *  - **입력**: `Pass::PipelineState` — *Material.Pass.Kind 가 결정한 완전한 GL state* (Stencil 포함)
 ...
 *  - 이전: `Set(want, stencilOverride)` (MeshRenderer 의 Stencil override 받음)
 *  - 현재: `Set(want)` — Pass::PipelineState 안에 *이미 Stencil 필드 통합 보유* (StencilEnable/Func/Ref/...)
 *
 *  ### 정통 매핑
...
namespace SJH
{
    ...
    {
    public:
        /// @brief Pass.PipelineState (Stencil 포함) 를 GL state 로 적용 (dirty check).
