 ...
 *    @c InvalidateStateCache -> @c ApplyRenderStateBlock(Opaque baseline) 순 (D-RS-1/D-RS-5).
 *    Stencil 도 함께 clear (@c DEPTH24_STENCIL8 포맷 가정 - Stencil 미사용 패스는 추가 비용 미미).
 *  - @c ApplyRenderStateBlock : 구 @c RenderStateBlockSetter 의 Stencil4/Depth3/Cull1/Blend2 dirty-apply 흡수
...
namespace SJH
{
	...
	{
		// ===================================================================================
		//  GL state machine dirty-apply 헬퍼 (D-RS-1 - 구 RenderStateBlockSetter 이식)
		...
		void SetStencilToggle(bool want, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void SetStencilFunc(const Pass::RenderStateBlock &s, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void SetStencilOp(const Pass::RenderStateBlock &s, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void SetStencilMask(const Pass::RenderStateBlock &s, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void ApplyStencil(const Pass::RenderStateBlock &s, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void SetDepthToggle(bool test, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void SetDepthMask(bool write, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void SetDepthFunc(GLenum func, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void ApplyDepth(const Pass::RenderStateBlock &want, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void ApplyCull(GLenum cullMode, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void SetBlendToggle(bool enable, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void SetBlendFunc(GLenum src, GLenum dst, Pass::RenderStateBlock &last, bool initialized)
		{
		...
		void ApplyBlend(const Pass::RenderStateBlock &want, Pass::RenderStateBlock &last, bool initialized)
		{
	...
	{
		// A-dedup: 동일 Program 이 이미 바인딩돼 있으면 glUseProgram skip (D-RS-1 캐시 규율 연장).
		// mStateInitialized=false(InvalidateStateCache 직후) 이면 가드를 통과시켜 강제 재바인딩 - 안전.
	...
	void DeviceContext::ApplyRenderStateBlock(const Pass::RenderStateBlock &want)
	{
	...
	{
		// 캐시 무효화 - 다음 ApplyRenderStateBlock 가 first-call 처럼 전체 강제 적용.
	...
	void DeviceContext::BeginFrame(RenderTarget &target)
	{
		...
		// (D-RS-5 - 구 SetDepthTest(true)+SetBlend(true) 명령형 토글 대체. 각 draw 가 자기 RenderStateBlock override.)
		InvalidateStateCache();
		ApplyRenderStateBlock(Pass::DefaultRenderStateBlockOf(Pass::RenderQueue::Opaque));
	}
