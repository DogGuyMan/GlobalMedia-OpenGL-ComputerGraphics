/**
 * @file gl_log.h
 * @brief GL 객체(셰이더/프로그램) InfoLog 조회 + GL 호출별 명시적 에러 검사.
 *
 * @details
 *  ### 책임
 *  - @c GLObjectLog - 셰이더 컴파일 / 프로그램 링크 / 검증(glValidateProgram) 결과 조회,
 *    기대 uniform/attribute 존재 여부 *program 별 1회 캐시* 검사.
 *  - @c GLDebug - @c glGenVertexArrays / @c glBindVertexArray / @c glGenBuffers /
 *    @c glBindBuffer / @c glBufferData / @c glEnableVertexAttribArray / @c glVertexAttribPointer
 *    각 호출 직후 @c glGetError 1:1 진단. 명세상 가능 에러 enum 별로 메시지 분기.
 *
 *  ### 비-책임
 *  - [X] GL 상태 덤프 - @c gl_state_log.h 의 @c GLStateLog::Dump.
 *  - [X] Mesh<->Shader contract 검증 - @c gl_validate.h 의 @c GLValidate 카테고리.
 *  - [X] uniform warn-once 중복 제거 - @c uniform_diagnostics.h 의 @c UniformDiagnostics.
 *
 *  ### 사용 패턴
 *  챕터/모듈은 관용적으로 `namespace diag = SJH::Diagnostics;` 별칭을 두고
 *  `diag::GLObjectLog::CheckShaderCompile(...)` 형식으로 호출.
 *
 * @note spdlog/fmt 의존 - diagnostics 모듈만 사용. 다른 모듈은 fmt 직접 참조 금지.
 */
...
namespace SJH::Diagnostics
{
     ...
     * @details
     *  컴파일/링크/검증 결과는 비동기 아니지만 별도 status 쿼리(@c glGetShaderiv /
     *  @c glGetProgramiv)만이 유일한 확인 수단. 그 반복 보일러플레이트를 본 클래스에 가둔다.
     *
     *  - `CheckShaderCompile` / `CheckProgramLink` - @c src/shader/ / @c src/program/ 에서 호출.
     *  - `CheckProgramValidate` - 디버그 빌드 draw 직전에만 (무거운 호출).
     *  - `CheckExpectedUniforms` / `CheckExpectedAttributes` - program 별 1회 캐시로 매 draw 안에 두어도 부담 0.
     */
    class GLObjectLog
    {
    public:
     ...
     *  - 내부에서 @c glGetError 1회 호출 -> GL 명세상 가능한 enum 들을 case 별로 분기해 보고.
     *  - 학습 단계에서 *어떤 호출이 어떤 에러를 낼 수 있는지* 코드에 명시되도록 풀어놓은 형태.
     *  - 콜백/매크로 방식(@c SJH_GL_CHECK) 대비 verbose 하지만 교육적 가치가 큼.
     *
     * @note 모든 함수는 호출 직전 GL 에러 큐가 비어있다고 가정 - 호출별 일관 적용 필수.
     */
    class GLDebug
    {
    public:
