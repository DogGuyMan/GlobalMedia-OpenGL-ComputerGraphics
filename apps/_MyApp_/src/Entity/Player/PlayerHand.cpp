...
namespace TopdownShooter::Entity
{
	...
	void PlayerHands::Update(float dt)
	{
		...
		// 화면(NDC) 거리 -> half-angle 보간. t=0(커서가 플레이어 화면위치 위) -> MAX(90deg: 양팔 180deg),
		// t=1(커서 화면 가장자리) -> MIN(7.5deg: 양팔 15deg). GetAimScreenT 는 이미 0~1 포화/정규화.
		const float t = mController->GetAimScreenT();
		float halfDeg = HAND_HALF_ANGLE_MAX + (HAND_HALF_ANGLE_MIN - HAND_HALF_ANGLE_MAX) * t;

		// 발사 핀치 - TweenPlayable 이 mFireBlend 를 1->0 으로 감쇠. blend 만큼 MIN(7.5deg)으로 좁힘.
		// blend=1: finalHalf=MIN(완전 핀치=15deg), blend=0: 거리기반 halfDeg.
	...
	{
		mFireBlend = 1.0f; // 클릭 즉시 완전 핀치(15deg)
