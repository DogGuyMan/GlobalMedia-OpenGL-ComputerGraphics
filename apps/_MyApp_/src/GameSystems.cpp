 ...
 * @brief @c TopdownShooter::GameSystems 구현 - 하위 시스템 라이프사이클 일괄 조율.
 *
 * @details
 *  ### 책임
 ...
 *  한 프레임의 시뮬레이션 단계 순서:
 *   1. @c GameSystems::Update(dt) - VFX 갱신 후 Physics @c Step (이 안에서 충돌 콜백 enqueue).
...
namespace TopdownShooter
{
	...
	void GameSystems::Init()
	{
		...
		spdlog::info("[GameSystems] init OK (Audio + VFX + Physics + WorldText)");
	}

	/// @copydoc GameSystems::Update
	...
	/// @copydoc GameSystems::Shutdown
	void GameSystems::Shutdown()
	{
		// 역순 - Physics 가 다른 시스템 참조 없으므로 임의 순서 OK 이지만 명시.
		mPhysics.Shutdown();
		mVFX.Shutdown();
		mAudio.Shutdown();
		spdlog::info("[GameSystems] shutdown OK");
	}
}
