 ...
 *  - @c VFXSystem / @c AudioSystem 과 평행한 위치의 게임 시스템 - @c GameSystems 가 값으로 보유.
 *
 *  ### 비-책임
...
{
    /**
     * @brief minogram BMFont 1개를 보유하는 얇은 시스템 (VFX/Audio 시스템 평행). @c GameSystems 보유.
     * @details 로드 성공 여부를 @c mLoaded 로 추적하고, 실패 시 @c GetFont 가 nullptr 을 반환한다.
     */
    class WorldTextSystem
    {
      public:
        /// @brief minogram BMFont 를 로드. @c GameSystems::Init 에서 1회 호출.
        /// @note 인자 없이 내부에서 ResourceRegistry::Get() 사용 - 호출자(GameSystems.cpp)가
