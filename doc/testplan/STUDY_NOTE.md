...
**증상**: 렌더가 되긴 되는데 화면에 "아무것도 안 보임". 셰이더 문제, VAO 문제 등 다른 원인을 먼저 의심하게 돼서 디버깅이 길어졌다.
...
**원인**: 두 개의 sampler (`tex1`/`tex2`) 가 서로 다른 의미(고정 vs 면별) 인데 FS 에서 **의도와 다른 쪽을 출력**. 셰이더를 작성한 본인조차 `tex1` / `tex2` 중 어느 게 면별인지 헷갈림.
...
### 1-6. [NEW] `glUniform*` 함수와 셰이더 타입 불일치 -> 조용히 거절 🔴
...
**이번 함정**: 행렬을 mat4 로 다루는 데 익숙해져서 "uniform 넘길 때는 `glUniformMatrix4fv` 쓰면 된다" 는 잘못된 패턴이 손에 배어버림. 실제로는 **uniform 함수가 셰이더 타입과 1:1 매칭** 되어야 한다.

**OpenGL uniform 함수 매핑 표** (반드시 외울 것):

| 셰이더 타입 | CPU 함수 | 주로 쓰는 데 |
|-----------|---------|-------------|
...
**현실**: 셰이더에 `uniform sampler2D tex1;` 이 있을 때, **"어느 unit 의 텍스처를 읽을지"** 와 **"그 unit 에 어떤 텍스처가 있는지"** 는 **서로 독립된 두 상태**. 둘 다 설정해야 실제 연결이 됨.
...
| # | 항목 | 확인 |
|---|------|------|
| 18 | 🔴 **`glUniform*` 함수가 셰이더 타입과 1:1 매칭**되는가? (vec2->`2fv`, mat4->`Matrix4fv`, sampler->`1i`) | |
...
1. 🔴 **`glUniform*` 은 셰이더 타입과 반드시 매칭** — mat4 함수로 vec2 업로드 같은 미스매치는 드라이버가 조용히 거절하고 uniform 은 기본값 (0) 에 머무름. 이게 "단색 텍스처" 와 "애니메이션 무반응" 같은 복합 증상으로 나타난다.
