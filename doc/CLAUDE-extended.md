...
> `.claude/CLAUDE.md`가 ~80줄 나침반으로 압축되며 이관된 상세 전문. 최신 나침반은 `.claude/CLAUDE.md`, 모듈별 상세는 `src/CLAUDE.md` 등 6개 모듈 CLAUDE.md + `ARCHITECTURE.md` + `doc/EngineAPI.md` 참조.
...
- **국면별 경계 오류**: 기능 배치·소유권 결정 국면 = 증분 패치형 오판 경계 → 기존 소유자 grep 선행 (`design-decision-discipline` §6.1). 마이그레이션·설계 국면 = "정통 패턴" 논변의 구조물 증식 경계 → 신규 구조물 신고 4항 (같은 스킬 §2.5).
...
- `doc/FMOD_Setup.md` — FMOD Core API 수동 설치 가이드 (독점 SDK 라 서브모듈 불가) + Dependency.cmake 등록 명세 (game_deps 자동 합류, POST_BUILD copy 의무)
