...
CODE_EXTS = {".py", ".ts", ".tsx", ".js", ".jsx", ".go", ".rs", ".java", ".kt", ".rb", ".php", ".sql", ".swift", ".cs",
             ".cpp", ".cc", ".cxx", ".h", ".hpp", ".inl", ".cmake", ".slang", ".vert", ".frag", ".glsl", ".geom", ".comp"}
    ...
    for r, dirs, files in os.walk(repo):
        ...
        for f in files:
            if f in CONTEXT_FILES:
                out.append(Path(r) / f)
    for extra in (repo / ".claude" / "CLAUDE.md", repo / ".claude" / "architecture.md"):
        if extra.exists():
            out.append(extra)
    return out
...
def score_d(repo: Path, context_files: list[Path]) -> CategoryScore:
    arch_paths = [repo / q for q in (
        "ARCHITECTURE.md", "docs/architecture.md", "docs/ARCHITECTURE.md",
        "docs/dependency-graph.md", "docs/data-flow.md",
    ) if (repo / q).exists()]
    has_arch = bool(arch_paths)
    has_mermaid = any(RE_MERMAID.search(read_text(p)) for p in [*context_files, *arch_paths])
...
def score_e(repo: Path, context_files: list[Path]) -> CategoryScore:
    ...
    for p in context_files:
        ...
        for ref in set(RE_PATH_REF.findall(text)):
            ...
            candidates = [repo / ref, p.parent / ref, repo / 'include' / ref, repo / 'src' / ref]
...
def score_g(repo: Path) -> CategoryScore:
    ...
    has_telemetry_hint = any(
        ...
        for p in (repo / "CLAUDE.md", repo / "README.md", repo / "AGENTS.md", repo / ".claude" / "CLAUDE.md")
        if p.exists()
    )
...
def render_markdown(report: Report) -> str:
    lines = []
    lines.append(f"# AI-Readiness Audit [instrument v2 2026-07-10] · {report.meta['repo']}")
    lines.append("")
